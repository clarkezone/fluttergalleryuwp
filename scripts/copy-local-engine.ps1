$binarydebugunoptsourcepath = '..\..\engine\src\out\winuwp_debug_unopt\'
...
if ((!$sourcepathdebugunopt) -or (!(Test-Path($sourcepathdebugunopt)))) {
    Write-Output "No flutter engine build found.  See http://s.clarkezone.io/flu4 for more info on how to build binaries of use the get-prebuilt-binaries.ps1 script "
    return
}
...
# # Update flutter windows binaries in each target project
# $targets = ".\TestbedUWP", ".\DadJokesUWP", ".\Flutter Particle CLock UWP", ".\HelloUWP", ".\FlutterGalleryUWP"
# foreach ($item in $targets) {
#     $targetdir = Join-Path -Path $srcdir -ChildPath $item
#     copy-item $fdlld $targetdir
#     copy-item $fpdbd $targetdir            
# }